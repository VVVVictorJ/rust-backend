...
1) 创建
   ...
   ```
   curl -X POST http://localhost:8001/stock-requests -H "Content-Type: application/json" -d '{"strategy_name":"demo","time_range_start":"2024-01-01"}'
   ```
2) 查询

   ```
   curl http://localhost:8001/stock-requests/1
   ```
3) 删除

   ```
   curl -X DELETE http://localhost:8001/stock-requests/1
   ```
